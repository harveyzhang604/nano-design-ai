...
async function uploadToR2(base64Data: string, prefix: string = 'tattoo'): Promise<string | null> {
  try {
...
export async function POST(req: Request) {
  try {
    const { imageUrl, tattooDesign, bodyPart = 'arm' } = await req.json();
    
    if (!imageUrl || !tattooDesign) {
      return NextResponse.json({ error: 'Image URL and tattoo design are required' }, { status: 400 });
    }
    ...
    // 尝试上传到 R2
    const r2Url = await uploadToR2(fullBase64, 'tattoo');
    ...
    console.error('Tattoo preview error:', error);
