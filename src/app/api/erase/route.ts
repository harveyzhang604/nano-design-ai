...
export async function POST(req: Request) {
  try {
    ...
    // 调用 Gemini API 进行智能擦除
    // 使用 inpainting 技术移除指定区域的物体
    const prompt = maskArea 
      ? `Remove the object in the marked area and fill it naturally with the surrounding background. Keep the rest of the image unchanged. Make the removal seamless and undetectable.`
      : `Intelligently detect and remove unwanted objects, people, or elements from this image. Fill the removed areas naturally with appropriate background. Keep the main subject intact and make the removal look natural.`;
